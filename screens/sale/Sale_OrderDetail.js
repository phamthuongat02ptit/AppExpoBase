...
const OrderDetail = ({ route, navigation, ...props }) => {
    ...
    const GetDMS_OrderDetail = async (MasterId = "", Isload = true) => {
        try {
            ...
            let result = await GetApis("DMS_OrderMaster", "GetDMS_OrderDetail", {
                id: MasterId,
    ...
    const getListCMP = async () => {
        try {
            let userStorage = await AsyncStorage.getItem("userInfo");
            let curUser = JSON.parse(userStorage);
            let resultCmp = await GetApis("dms/GetListCicmpyByUserDapper", "", {
                username: curUser.UserName,
            });
            if (resultCmp) {
                let lstCmp = [];
            ...
            let resultAgency = await GetApis("dms/GetListDMSCustomerLookup", "", {
                username: curUser.UserName,
            });
            if (resultAgency) {
                let lstAgency = [];
